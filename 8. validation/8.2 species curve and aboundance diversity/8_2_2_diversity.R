rm(list = ls())
pacman::p_load(dplyr, ggplot2, vegan, metagenomeSeq, magrittr, car, tidyverse, rstudioapi)
...
df_abund <- read.csv('../data/species_abund_rel_sample_filtered_id.csv', header = T, row.names = 1)
df_abund <- df_abund[, which(colnames(df_abund) %in% df_maped$sample)]
data_mr_type <- newMRexperiment(df_abund) # css normalization
...
df_cates <- read.csv("../data/meta290.csv", header=T)[,c(1,12:16)]
