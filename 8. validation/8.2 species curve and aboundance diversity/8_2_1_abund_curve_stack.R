rm(list = ls())
pacman::p_load(openxlsx, dplyr, ggplot2, vegan, reshape2, viridis, rstudioapi)
...
df_curve <- read.csv('source/abund_rel.csv', header = T, row.names = 1)
samples <- rownames(read.csv('../data/metadata.csv', header = T, row.names = 1))
df_curve <- filter(df_curve, rownames(df_curve) %in% samples)
sp <- specaccum(df_curve, method='random')

pdf('result/curve.pdf', width = 10, height = 8)
plot(sp, ylim=c(50, 5500), 
