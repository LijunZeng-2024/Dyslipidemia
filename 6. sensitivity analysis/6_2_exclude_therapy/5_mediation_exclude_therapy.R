...
mediation_func <- function(cate){
  ...
  for(species in cate_species){
    ...
    formular_M <- paste0(species, '~score+age+Sex+Ethnicity+Education+Marriage')
    fitM <- lm(as.formula(formular_M), data=df)
    
    formular_Y <- paste0(cate, ' ~ score+', species,'+age+Sex+Ethnicity+Education+Marriage')
