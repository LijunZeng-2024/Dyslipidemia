...
#str(df_meta)
df_species_abund <- read.table(paste0('../3. differential gut microbiota/', result_dir, '/Dyslipidemia/features/filtered_data_norm_transformed.tsv'), header = T, sep = '\t' ,stringsAsFactors = F, row.names = 1)
...
mediation_func <- function(cate){
  ...
  cate_species = (read.table(paste0('../3. differential gut microbiota/',result_dir, '/', cate, '/significant_results.tsv'), header = T, sep = '\t' ,stringsAsFactors = F))$feature
  ...
  for(species in cate_species){
    ...
    formular_M <- paste0(species, '~score')
    fitM <- lm(as.formula(formular_M), data=df)
    
    formular_Y <- paste0(cate, ' ~ score+', species)
